...
Python is a unfamiliar thing.

[crypto challenge]
Cryptography is a unfamiliar thing.
Caesar cipher and substitution cipher is a familiar thing.
...
Instead of drinking a drink (called thisdrink):
	say "[one of]Lovely [thisdrink]. Still hot too![or]You savour the [thisdrink], drinking it slowly.[or]Just what you needed.[at random]";
...
Coffee, coke, and tea are kinds of drink.
...
The red fish and the shiny red keycard are fixed in place.
...
Ground floor stairs is north of ULU Entrance. "[generic stairwell desc]. [paragraph break]The only way is UP! or something like that. [paragraph break]There is a message board lining the walls of the stairwell."
	...
	otherwise:
		if the chosen number is 1:
			if player carries the blueberry muffin:
				say "You open your hand to reveal yet-another-blueberry muffin. You put it back as one is enough.";
		...
		if the chosen number is 2:
			if player carries the plain muffin:
				say "You may have hands like a surgeon when it comes to getting muffins out of a stack like this, but you've just picked up another plain muffin. You put it back as carrying one around is bad enough. Your bowels will thank you later.";
				now the chosen number is 0;
			otherwise:
...
To say manager refusal:
	say "[one of]'Hey!' barks a hitherto-unseen person from behind you. 'The muffins may be 'all you can eat' buffet, but save some for the others!'[or]You are conscious of a disapproving stare coming from across the table, so you refrain.[stopping]"
...
After taking the strange code fragment:
	say "You take the paper, but at a glance you can tell that it is encrypted - you can't tell if it will contain important information or a recipe for good caesar salad. Actually, maybe it is worth decoding then.[make cryptography known]".
...
The ask-suggestions of Ben O'Steen are { programming languages, python, cryptography, ROT code, Ruby }.
The unsuggested-topics of Ben O'Steen are {rickroll}.

Response of Ben O'Steen when asked about python:
	say "'I think it's a wonderful language - whether it is the 'best' one to learn is really dependent on what you want to get done but I think it's a great way to start.'"

Response of Ben O'Steen when asked about Ruby:
	say "'Ruby, yep, a fine choice of language, lots of shiny things going on with that and in that community. Not my cup of tea though, too much punctuation and odd phrasings for me.'"

Response of Ben O'Steen when shown the strange code fragment:
	say "'You found one of my ciphers! Have you solved it yet? Do you get it?' he asks. [make rickroll known]"
...
Response of Ben O'Steen when asked about programming languages:
	say "He replies 'To be honest, they are all the same. Pick one, learn it by writing actual code and most importantly, learn its libraries and how the community works. [first time][make python known][make ruby known][only]I have to say though, I have a real soft spot for python. Does the job, clear, easy, nice. I guess I could say the same things about Ruby, but I learnt python first.'";
	show topic suggestions implicitly;

Response of Ben O'Steen when asked about cryptography:
	say "He looks up at the mention of cryptography. 'Is someone is doing a lightning talk on it this year? It would be cool if someone was... What do you want to know about it? Do you know about the simple stuff like [add caesar cipher ask suggestion]caesar or substitution[add substitution cipher ask suggestion] ciphers or more advanced, shiny codes?'";
	show topic suggestions implicitly;

Response of Ben O'Steen when asked about caesar cipher:
	say "'Imagine the alphabet you want to use written out twice, one row above another. For example :[paragraph break]A B C D E F G H I...[line break]A B C D E F G H I...[paragraph break]By shifting the second row to the right or left by some number of letters, say, 3 letters to the right, you can create a caesar cipher encoder:[paragraph break]A B C D E F G H I...[line break]X Y Z A B C D E F...[line break](Note that the letters wrap around.)[paragraph break] To encode a letter, you find it on the top row and use the letter under it. In this way, BED becomes YBA, HAG becomes EXD and so on. ROT-13 ('rotate by 13') is a well known variation on the caesar cipher. Pretty easy to code a ROT in python too.[make ROT code known]'";
	show topic suggestions implicitly;
...
Response of Ben O'Steen when asked about ROT code:
	...
	otherwise:
		say "[make python known]He looks puzzled. 'You really need me to tell you? Well, you did feed me, so let's see, you just need to turn each letter into a number, add or subtract the shift amount to it, wrap that answer around if necessary and turn it back to text... this should do the trick in python...' He opens a new terminal on his laptop and begins to type up the following code:[paragraph break][italic type]    def encode(txt, n): return [quotation mark][quotation mark].join([bracket]chr((ord(c)-ord('a')+n) % 26 + ord('a')) for c in txt.lower()[close bracket])[roman type][paragraph break]'Of course, this doesn't handle spaces and punctuation, but you get the idea!'"
